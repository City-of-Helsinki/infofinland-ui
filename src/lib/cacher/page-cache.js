...
const parseKey = (key) => {
  try {
    ...
    logger.error('Error while parsin cache key', e ,{cacheKey:key,CACHE_NAME})
    return []
  }
...
const getKey= (keyObj) => {
  try {
    ...
    logger.error('Error while creating cache key', e ,{cacheKey:keyObj, CACHE_NAME})
    return []
  }
...
cache.on('expired',async (expiredKey)=>{
  if(getConfig().serverRuntimeConfig.CACHE_REPOPULATE === '1') {
    console.debug(' page cache key expiredcache repopulation is ON', {cacheRepopulate:'1', expiredKey})
    const params = parseKey(expiredKey)
    logger.info('Cache entry expired',{cacheKey:expiredKey, params, cacheName: CACHE_NAME })
    const fresh = await getNode(params)
    if(fresh){
      logger.info('Refreshing entry ',{cacheKey:expiredKey,cacheName: CACHE_NAME })
